...
function update_sum() {
    ...
    for (var i = 0; i < sum.length; i++) {
        sum[i].innerHTML = " - " + expense_per_category[i] + " $ spent";
    }
}

function add_expense() {
    var amount = document.getElementsByClassName("form-input")[0].value;
    var categorySelect = document.getElementsByClassName("form-input")[1];
    ...
    var selected = categorySelect.options[category].value;
...
function add_category(){
    ...
    if(new_category_value) {
        ...
        new_category_span.innerHTML = `<span>${new_category_value}<span class="sum"> - ${expense_per_category[count]} $ spent</span></span>`;
